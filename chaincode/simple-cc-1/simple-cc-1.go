...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
  ...
  if fn == "putState" {
    return s.PutState(APIstub, args)
  } else if fn == "getState" {
...
func (s *SmartContract) PutState(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
  if (len(args) < 2) {
    return shim.Error(fmt.Sprintf("Need at least 2 arguments, got %d", len(args)))
  }
...
func (s *SmartContract) GetState(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
  if (len(args) < 1) {
    return shim.Error(fmt.Sprintf("Need at least 1 argument"))
  }
